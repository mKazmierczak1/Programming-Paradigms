...
let q = ref (Queue_Fun_Double_List.empty());;
  ...
  begin
	...
	1 ->
	  begin
	    ...
	    q := Queue_Fun_Double_List.empty();
	  end  
      | 2 ->
	  begin
            print_string "Element = ";
            q := Queue_Fun_Double_List.enqueue(read_int(), !q);
	  end
      | 3 ->
	  begin
            q := Queue_Fun_Double_List.dequeue !q;
            print_string "Dequeue\n";
	  end
      | 4 ->
	  begin
	    begin
               try print_int (Queue_Fun_Double_List.first (!q)) with
		 Queue_Fun_Double_List.Empty m -> print_string ("Exception: "^m);
	    end;
	    print_newline();
	  end  
      | 5 ->
	    print_string ("Queue is "^(if Queue_Fun_Double_List.isEmpty !q then "" else "not ")^"empty.\n");
      | 6 ->
	    quit := true
      | _ ->
